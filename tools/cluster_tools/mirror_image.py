...
def send_pexpect_command(child, command, prompt):
    child.sendline(command)
    try:
        child.expect(prompt, timeout=60)  # Increase the timeout to 60 seconds
    except pexpect.exceptions.TIMEOUT:
        print("Timeout exceeded while waiting for the prompt.")
        print("Buffer content:", child.before.decode('utf-8'))
